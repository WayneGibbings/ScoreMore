...
export const Team: React.FC<TeamProps> = ({
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-md p-4 border-t-4 ${isEditMode ? 'ring-2 ring-blue-500' : ''}`}
      style={{ borderTopColor: `var(--color-${team.color}-500, #${getColorHex(team.color)})` }}
    >
      ...
      {team.players.length > 0 ? (
        ...
        <div className="text-center py-4 text-gray-500">
          {isEditMode
            ? "Enter the first player's name below"
      ...
      {isEditMode && (
        <form onSubmit={handleAddPlayer} className="mt-4">
          <div className="flex flex-col space-y-2">
            <label htmlFor={`add-player-${team.id}`} className="sr-only">
              Add player name
            </label>
            <input
              type="text"
            ...
            <button
              type="submit"
              className="w-full bg-gray-800 text-white px-4 py-2 rounded text-sm disabled:bg-gray-400"
